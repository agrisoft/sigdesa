...
$('#listdatadasar').click(function(e) {
    ...
    window.dastext = $(e.target).text()
    $("#dasartext").text(dastext);
...
$('#listvariabel').click(function(e) {
    ...
    // window.dasariddata = iddata;
    tabeltipe = 'dasar';
    $("#dasartext").text(dastext + ' / ' + $(e.target).text());
    if (zoom_state == 0) {
...
function load_prov() {
    d3.json("static/data/admin_kab.topojson", function(error, admin_kab) {
        ...
        g.append("g")
            ...
            .on("mousemove", function(d) { return tooltip.style("top", (event.pageY - 80) + "px").style("left", (event.pageX - 260) + "px").text(d.properties.WADMKK); })
...
function load_kab(kodekab) {
    d3.json("static/data/" + kodekab + ".topojson", function(error, kab) {
        ...
        g.append("g")
            ...
            .on("mousemove", function(d) { return tooltip.style("top", (event.pageY - 80) + "px").style("left", (event.pageX - 260) + "px").text(d.properties.NAMOBJ); })
            .on("mouseout", function(d) { return tooltip.style("visibility", "hidden"); });
...
function load_inital() {
    ...
    setTimeout(function() {
        ...
        d3.json("static/data/p_jawa.topojson", function(error, p_jawa) {
            if (error) throw error;

            g.append("g")
                .attr("id", "provinsi")
                .selectAll("path")
                .data(topojson.feature(p_jawa, p_jawa.objects.p_jawa).features)
                .enter().append("path")
                .attr("d", path)
                .attr("class", "sekitaran")
                .attr("provinsi", function(d) { return d.properties.NAMA_1 })
...
cc.on('sclick', function(el) {
    ...
    var switched = false;
    if (d) {
        if (d.properties.KDBBPS == undefined) {
            dzoomed = false;
        } else {
            dzoomed = true;
        }
        ...
        if (zoom_state == 0) {
            ...
            $('#kdesatext1').text("JAWA TENGAH / " + d.properties.WADMKK);
            $('#kdesatext2').text("JAWA TENGAH / " + d.properties.WADMKK);
        } else {
            console.log("B")
            ...
            layer_zoomin(d);
            layer_transform();
            console.log(d.properties, lastgeom.properties)
            if (zoom_state == 1) {
                $('#kdesatext').text("JAWA TENGAH / " + d.properties.WADMKK);
                $('#kdesatext1').text("JAWA TENGAH / " + d.properties.WADMKK);
                $('#kdesatext2').text("JAWA TENGAH / " + d.properties.WADMKK);
                ...
                $('#kdesatext1').text("JAWA TENGAH / " + d.properties.WADMKK + " / " + d.properties.NAMOBJ);
                $('#kdesatext2').text("JAWA TENGAH / " + d.properties.WADMKK + " / " + d.properties.NAMOBJ);
            }
            if (maptipe == 4) {
                $('#masterprofile').show();
                $('#md_judul').hide();
...
cc.on('dblclick', function(el) {
    ...
    if (zoom_state == 1) {
        ...
        $('#kdesatext1').text("JAWA TENGAH");
        $('#kdesatext2').text("JAWA TENGAH");
        if (tabeltipe == 'dasar') {
...
function getColor(color_domain, colors, value) {
    //value from 0 to 1
    ...
    $.get("/grafmodel1c3" + '?tabel=' + tabel + '&tahun=' + tahun + '&kode=' + kodekab,
        function(data) {
            ...
            $.get("/kategoridesc", {
                ...
                // colors.push("999999");
                // console.log(color_domain, colors, legend_labels);
                ...
                for (i = 0; i < data[0]['v_color'].split(',').length; i++) {
                    ...
                    // legend_labels.push(data[0]['r_break'].split(',')[i]);
                    // colors.push(data[0]['v_color'].split(',')[i]);
                    console.log(color_domain, legend_labels, colors);
                    ...
                    legend.append("rect")
                        ...
                        .style("stroke", "#DDDDDD")
                        .style("stroke-width", "1")
                        .style("fill", function(d, i) { return getColor(color_domain, colors, d) })
                        .style("opacity", 0.8);
                    legend.append("text")
                        .attr("x", 50)
                        .attr("y", function(d, i) { return (height - (i * ls_h) - ls_h - 4 - 20); })
